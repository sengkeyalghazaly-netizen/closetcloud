  ...
  if (card.type === "outfit") {
    ...
    return (
      <Card className="mt-2" style={{ border: `1.5px solid ${T.mint}` }}>
        <div className="flex items-center gap-1.5 mb-2"><Sparkles size={14} color={T.lavenderDeep} /><p className="text-xs font-bold" style={{ color: T.navy }}>{card.title || "Outfit dari Ajax"}</p></div>
...
export function KaiScreen({ items, setItems, chat, setChat, plan, usage, useQuota, onUpgrade, onBack }) {
  ...
  const send = async (text) => {
    ...
    if (!useQuota("ai_chat", "Chat Ajax tanpa batas")) return;
    setInput("");
    setFailedMsg(null);
  ...
  return (
    <div className="pb-24 flex flex-col" style={{ height: "100vh" }}>
      <Header title="Ajax — AI Stylist" subtitle="Tanya apa saja soal lemarimu" onBack={onBack} />
      <QuotaBanner remaining={left} limit={FREE_LIMITS.ai_chat} label="pesan Ajax" onUpgrade={onUpgrade} />
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 pb-4">
        {chat.length === 0 && !loading && (
          <div className="flex flex-col items-center pt-6">
            ...
            <p className="text-sm text-center mb-5 max-w-xs" style={{ color: T.navySoft }}>Halo! Aku Ajax. Aku sudah lihat isi lemarimu — mau mulai dari mana?</p>
        ...
        {failedMsg && !loading && (
          <div className="flex items-center gap-2 mt-3 px-3 py-2.5 rounded-xl" style={{ background: "#FCEEE9" }}>
            ...
            <p className="text-xs flex-1" style={{ color: T.navy }}>Ajax lagi susah dihubungi. Cek koneksi lalu coba lagi.</p>
      ...
      <div className="fixed bottom-16 left-0 right-0 max-w-md mx-auto px-4 pb-2 pt-2" style={{ background: T.bg }}>
        <div className="flex gap-2 items-center rounded-2xl px-3 py-2" style={{ background: T.white, border: "1px solid #E3E6F0" }}>
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()}
            placeholder="Tanya Ajax soal outfit, donasi, DIY…" className="flex-1 outline-none text-sm bg-transparent" style={{ color: T.navy }} />
